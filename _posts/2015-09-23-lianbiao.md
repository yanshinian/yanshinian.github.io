...
```
Node *q = NULL;
Node *header = NULL; // 头指针
for (int i = 0; i < 10; i++) {
    // 動態申請一個空間，用來存放一個節點，并用臨時指針p指向這個節點
    Node *p = (Node *)malloc(sizeof(Node));
    p->data = i;// 將數組存放到當前節點的data域中
    if (header == NULL) {
        header = p;// 如果這是第一個創建的節點，則將頭指針指向這個節點
    } else {
        q->next = p;//如果不是第一個創建的節點，則將上一個節點的後繼指針指向當前節點
    }
    q = p;//指針q也指向當前節點
}
// 輸出鏈表總的所有數
